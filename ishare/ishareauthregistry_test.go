...
func TestGetDelegationEvidence(t *testing.T) {
	...
	tests := []test{
		...
		{"If AR's delegation endpoint responds 404, return a 404.", getDefaultTestAR(), getTestKey(), getTestCerts(), map[string]*http.Response{"http://fiware.org/connect/token": tokenResponse(), "http://fiware.org/delegation": notFoundResponse()}, map[string]*http.Response{}, map[string]error{}, true, nil, model.HttpError{Status: http.StatusNotFound}},
