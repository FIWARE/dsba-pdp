...
func TestIShareVerify(t *testing.T) {
	...
	tests := []test{
		/*	{"Allow subjects if no role claim is configured.", []model.Claim{}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{}}, "did:test:issuer", true, model.HttpError{}},
			...
			{"Allow subjects if claim allows the assigned role.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedStringValue("CUSTOMER")}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"CUSTOMER"}}}}, "did:test:issuer", true, model.HttpError{}},
			{"Allow if more roles are allowed.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedCustomerValue(), getAllowedEmployeeValue()}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"EMPLOYEE"}}}}, "did:test:issuer", true, model.HttpError{}},
			...
			{"Reject if claim has different role allowed values.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedEmployeeValue()}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"CUSTOMER"}}}}, "did:test:issuer", false, model.HttpError{}},
			{"Reject if subject has additional roles.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedEmployeeValue()}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"CUSTOMER", "EMPLOYEE"}}}}, "did:test:issuer", false, model.HttpError{}},
		...
		*/{"Allow subjects if role-provider is one of the allowed by the claim.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedStringValue("{\"providerId\":\"EU.EORI.HAPPYPETS\"}"), getAllowedStringValue("{\"providerId\":\"EU.EORI.PACKETDELIVERY\"}")}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"TEST"}, Provider: "EU.EORI.HAPPYPETS", Target: "did:test:issuer"}}}, "did:test:issuer", true, model.HttpError{}},
		...
		/*	{"Reject subjects if role-provider is not of the allowed by the claim.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedStringValue("{\"providerId\":\"EU.EORI.PACKETDELIVERY\"}")}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"TEST"}, Provider: "EU.EORI.HAPPYPETS", Target: "did:test:issuer"}}}, "did:test:issuer", false, model.HttpError{}},
			{"Reject subjects if no role for the issuer is configured and the role is not allowed by the general list.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedStringValue("CUSTOMER")}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"TEST"}, Target: "did:test:another-issuer", Provider: "EU.EORI.HAPPYPETS"}}}, "did:test:issuer", false, model.HttpError{}},

			{"Allow subjects if role and provider are allowed.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedStringValue("{\"providerId\":\"EU.EORI.PACKETDELIVERY\", \"name\":[\"CUSTOMER\"]}")}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"CUSTOMER"}, Provider: "EU.EORI.PACKETDELIVERY", Target: "did:test:issuer"}}}, "did:test:issuer", true, model.HttpError{}},
			{"Allow subjects if role, provider and ar are allowed.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedStringValue("{\"providerId\":\"EU.EORI.PACKETDELIVERY\", \"name\":[\"CUSTOMER\"]}")}}, {Name: AR_KEY, AllowedValues: &[]model.AllowedValue{getAllowedStringValue("EU.EORI.PACKETDELIVERY")}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"CUSTOMER"}, Provider: "EU.EORI.PACKETDELIVERY", Target: "did:test:issuer"}}, IShareCredentialsSubject: &model.IShareCredentialsSubject{AuthorizationRegistries: &map[string]model.AuthorizationRegistry{"EU.EORI.PACKETDELIVERY": {}}}}, "did:test:issuer", true, model.HttpError{}},

			{"Deny subjects if role is allowed for different provider, but provider not.", []model.Claim{{Name: ROLES_KEY, AllowedValues: &[]model.AllowedValue{getAllowedStringValue("{\"providerId\":\"EU.EORI.OTHERPROVIDER\", \"name\":[\"CUSTOMER\"]}")}}}, model.CredentialSubject{Id: "mySubject", Roles: []model.Role{{Name: []string{"CUSTOMER"}, Provider: "EU.EORI.PACKETDELIVERY", Target: "did:test:issuer"}}}, "did:test:issuer", false, model.HttpError{}},
		*/}
